  ...
  for (let i = 0; i < workHoursIdArr.length; i++) {
    ...
    if ($("hoursId")) {
      ...
      if (time.length === 4) {
        ...
        console.log("HOURNUMS: " + hourNUMA, hourNUMB);
...
$(".saveBtn").on("click", function () {
  let rowId = $(this).parent(".row").attr("id");

  let divId = $(this).parent(".row").attr("id").replace("row-", "");
  ...
  if (
    ...
    let text = $(".form-control").val().trim();
